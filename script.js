...
_str(v,l,i){const lmn=l.lenMin||1,lmx=l.lenMax||10;const len=i===0?lmn:i===1?lmx:this._ri(lmn,lmx);const cs={az:'abcdefghijklmnopqrstuvwxyz',AZ:'ABCDEFGHIJKLMNOPQRSTUVWXYZ','09':'0123456789',mixed:'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'};if((v.charset||'az')==='mixed'&&v.charsetRatio){const r=v.charsetRatio;const pools=[{chars:'0123456789',pct:r.digits||0},{chars:'abcdefghijklmnopqrstuvwxyz',pct:r.lower||0},{chars:'ABCDEFGHIJKLMNOPQRSTUVWXYZ',pct:r.upper||0}];let s='';for(let j=0;j<len;j++){const roll=Math.random()*100;let cum=0,pool=pools[pools.length-1].chars;for(const p of pools){cum+=p.pct;if(roll<cum){pool=p.chars;break}}s+=pool[Math.floor(Math.random()*pool.length)]}return s}const c=cs[v.charset||'az'];let s='';for(let j=0;j<len;j++)s+=c[Math.floor(Math.random()*c.length)];return s}
...
_addVar(lineId,name='',type='integer',lengthRef=null,charset='az',pattern='random'){this.varCounter++;const container=document.querySelector(`[data-line-vars="${lineId}"]`);if(!container)return;const sts=this._getSubtasks();const row=document.createElement('div');row.className='var-row';let chips='';sts.forEach((st,i)=>{const dl=DLIM[i]||DLIM[DLIM.length-1];const cc='st-'+Math.min(st.id,5);chips+=`<div class="constraint-chip ${cc}" data-st-id="${st.id}"><span class="st-label">ST${st.id}</span><input type="number" class="cst-min" value="${dl.min}"><span class="sep">—</span><input type="number" class="cst-max" value="${dl.max}"></div>`});const vars=this._getAllVarNames();const refO=vars.map(n=>`<option value="${n}" ${n===lengthRef?'selected':''}>${n}</option>`).join('');row.innerHTML=`<input type="text" class="var-name-input" value="${name}" placeholder="Tên"><select class="var-type-select"><option value="integer" ${type==='integer'?'selected':''}>Số nguyên</option><option value="float" ${type==='float'?'selected':''}>Số thực</option><option value="array" ${type==='array'?'selected':''}>Mảng</option><option value="string" ${type==='string'?'selected':''}>Chuỗi</option></select><div class="var-constraints-inline">${chips}</div><button class="btn-danger-sm btn-rm-var">✕</button>`;const extraArr=document.createElement('div');extraArr.className='var-extra-row '+(type==='array'?'':'hidden');extraArr.innerHTML=`<label>Dài:</label><select class="arr-length-ref"><option value="">Tự cấu hình</option>${refO}</select><label>Pattern:</label><select class="arr-pattern"><option value="random">Random</option><option value="ascending">Tăng</option><option value="descending">Giảm</option></select>`;const extraStr=document.createElement('div');extraStr.className='var-extra-row '+(type==='string'?'':'hidden');extraStr.innerHTML=`<label>Ký tự:</label><select class="str-charset"><option value="az">a-z</option><option value="AZ">A-Z</option><option value="09">0-9</option><option value="mixed">Hỗn hợp</option></select><div class="charset-ratio-group hidden"><label>Tỉ lệ:</label><div class="charset-ratio-inputs"><span class="ratio-label">🔢 Số</span><input type="number" class="ratio-digits" value="34" min="0" max="100" step="1"><span>%</span><span class="ratio-label">🔡 thường</span><input type="number" class="ratio-lower" value="33" min="0" max="100" step="1"><span>%</span><span class="ratio-label">🔠 HOA</span><input type="number" class="ratio-upper" value="33" min="0" max="100" step="1"><span>%</span><span class="ratio-total-badge">= 100%</span></div></div>`;const charsetSel=extraStr.querySelector('.str-charset');const ratioGroup=extraStr.querySelector('.charset-ratio-group');charsetSel.onchange=function(){ratioGroup.classList.toggle('hidden',this.value!=='mixed')};const ratioInputs=extraStr.querySelectorAll('.ratio-digits,.ratio-lower,.ratio-upper');const totalBadge=extraStr.querySelector('.ratio-total-badge');ratioInputs.forEach(inp=>inp.oninput=()=>{const d=parseInt(extraStr.querySelector('.ratio-digits').value)||0;const l=parseInt(extraStr.querySelector('.ratio-lower').value)||0;const u=parseInt(extraStr.querySelector('.ratio-upper').value)||0;const sum=d+l+u;totalBadge.textContent='= '+sum+'%';totalBadge.style.color=sum===100?'#10b981':'#f87171'});row.appendChild(extraArr);row.appendChild(extraStr);row.querySelector('.var-type-select').onchange=function(){extraArr.classList.toggle('hidden',this.value!=='array');extraStr.classList.toggle('hidden',this.value!=='string')};row.querySelector('.btn-rm-var').onclick=()=>{row.remove();this._validateForm()};container.appendChild(row);this._validateForm()}
